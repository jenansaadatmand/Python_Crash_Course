...
class ElectricCar(Car): # Define child class, the name of the parent class must be included in parentheses in the definition of a child class
    ...
    def __init__(self, make, model, year): # __init__() takes the information required to make a Car instance
        ...
        """
        super().__init__(make, model, year) # Inheritance, super().__init__() function allows to call a method from the parent class, tells Python to call the __init__() method from Car, which gives an ElectricCar instance all the attributes defined in that method. Superclass (Parent class) and subclass (child class) convention. no : at the end
        self.battery = Battery()  # Add a new attribute called self.battery, this line tells Python to create a new instance of Battery (with a default size of 75, because we're not specifying a value) and assign that instance to the attribute self.battery. eg of an instance from another class used as an attribute
